# マイクロサービスアプリケーション

Java 11 + Payara + PostgreSQL + Vue 3 で構築されたマイクロサービスアーキテクチャの会員管理アプリケーション

## 📋 概要

このアプリケーションは、ユーザー管理と認証機能を持つマイクロサービスで構成されています。ユーザーはログインして自分のアカウント情報とポイント残高を確認できます。

## 🏗 アーキテクチャ

### マイクロサービス構成

```
┌─────────────┐
│  Frontend   │  Vue 3 + Tailwind CSS (Port: 3000)
│  (Vue 3)    │
└──────┬──────┘
       │
       ↓
┌──────────────┐
│     BFF      │  Backend For Frontend (Port: 8082)
│ (Java/Payara)│
└──────┬───────┘
       │
       ├─────────────────┬─────────────────┬─────────────────┐
       ↓                 ↓                 ↓                 ↓
┌─────────────┐   ┌─────────────┐   ┌─────────────┐   ┌─────────────┐
│User Service │   │Auth Service │   │Point Service│   │  PostgreSQL │
│ (Port: 8080)│   │ (Port: 8081)│   │ (Port: 8083)│   │ (Port: 5432)│
└─────────────┘   └─────────────┘   └─────────────┘   └─────────────┘
       │                 │                 │
       │                 │                 │
       ↓                 ↓                 ↓
┌──────────────┐   ┌──────────────┐   ┌──────────────┐
│user_service_ │   │auth_service_ │   │point_service_│
│     db       │   │     db       │   │     db       │
└──────────────┘   └──────────────┘   └──────────────┘
```

### サービス一覧

| サービス         | ポート | 説明                     |
|------------------|-------|--------------------------|
| Frontend         | 3000  | Vue 3 フロントエンド      |
| BFF              | 8082  | Backend For Frontend     |
| User Service     | 8080  | ユーザー管理             |
| Auth Service     | 8081  | 認証・セッション管理     |
| Point Service    | 8083  | ポイント管理             |
| PostgreSQL       | 5432  | データベース             |

## 🚀 クイックスタート

### 前提条件

- Docker & Docker Compose
- VS Code with Dev Containers extension

### 重要: 初回セットアップ

**Maven 3.9.9（Java 11対応）とPayara Microをインストールするため、DevContainerの再ビルドが必要です。**

📖 **詳細な手順は [クイックスタートガイド](./docs/QUICKSTART.md) を参照してください**

### 起動方法

1. **リポジトリのクローン**
```bash
git clone <repository-url>
cd my-ws-demo
```

2. **Dev Containerで開く**
```bash
# VS Codeで開く
code .
# コマンドパレット (Ctrl+Shift+P) から
# "Dev Containers: Reopen in Container" を選択
```

3. **各サービスをビルド**

**重要**: まずDevContainerを再ビルドしてMaven 3.9.9（Java 11対応）とPayara Microをインストールしてください。

VS Codeのコマンドパレット (Ctrl+Shift+P) から:
```
Dev Containers: Rebuild Container
```

再ビルド後、各サービスをビルド：

ユーザーサービス:
```bash
cd /workspaces/my-ws-demo/src/user-service
mvn clean package
```

認証サービス:
```bash
cd /workspaces/my-ws-demo/src/auth-service
mvn clean package
```

BFF:
```bash
cd /workspaces/my-ws-demo/src/bff
mvn clean package
```

ポイントサービス:
```bash
cd /workspaces/my-ws-demo/src/point-service
mvn clean package
```

4. **各サービスを起動**

ターミナル1 - ユーザーサービス:
```bash
cd /workspaces/my-ws-demo/src/user-service
java -jar /opt/payara-micro.jar --deploy target/user-service.war --port 8080
```

ターミナル2 - 認証サービス:
```bash
cd /workspaces/my-ws-demo/src/auth-service
java -jar /opt/payara-micro.jar --deploy target/auth-service.war --port 8081
```

ターミナル3 - ポイントサービス:
```bash
cd /workspaces/my-ws-demo/src/point-service
java -jar /opt/payara-micro.jar --deploy target/point-service.war --port 8083
```

ターミナル4 - BFF:
```bash
cd /workspaces/my-ws-demo/src/bff
POINT_SERVICE_URL="http://localhost:8083" java -jar /opt/payara-micro.jar --deploy target/bff.war --port 8082
```

ターミナル5 - フロントエンド:
```bash
cd /workspaces/my-ws-demo/src/frontend
npm install
npm run dev
```

5. **アプリケーションにアクセス**

ブラウザで http://localhost:3000 を開く

## 👤 テストユーザー

以下のユーザーでログインできます（ユーザー名またはUUIDでログイン可能）:

| UUID | 氏名 | ユーザー名 | パスワード | ポイント |
|----------|------|-----------|-----------|---------|
| 123e4567-e89b-12d3-a456-426614174000 | 田中太郎 | tanaka_taro | password123 | 1500 |
| 123e4567-e89b-12d3-a456-426614174001 | 鈴木花子 | suzuki_hanako | password123 | 2300 |
| 123e4567-e89b-12d3-a456-426614174002 | 山田次郎 | yamada_jiro | password123 | 500 |
| 123e4567-e89b-12d3-a456-426614174003 | 佐藤優希 | sato_yuki | password123 | 3200 |
| 123e4567-e89b-12d3-a456-426614174004 | 高橋舞 | takahashi_mai | password123 | 1000 |

**ログイン方法:**
- ユーザー名で: `tanaka_taro` + `password123`
- UUIDで: `123e4567-e89b-12d3-a456-426614174000` + `password123`

## 📁 プロジェクト構造

```
my-ws-demo/
├── .devcontainer/          # Dev Container設定
│   ├── devcontainer.json
│   ├── docker-compose.yml
│   ├── Dockerfile
│   └── init-db.sql
├── src/
│   ├── user-service/       # ユーザー管理サービス
│   │   ├── database/
│   │   │   ├── schema.sql
│   │   │   └── seed.sql
│   │   ├── src/
│   │   └── pom.xml
│   ├── auth-service/       # 認証サービス
│   │   ├── database/
│   │   │   ├── schema.sql
│   │   │   └── seed.sql
│   │   ├── src/
│   │   └── pom.xml
│   └── bff/                # Backend For Frontend
│       ├── src/
│       └── pom.xml
├── frontend/               # Vue 3 フロントエンド
│   ├── src/
│   ├── package.json
│   └── vite.config.js
└── docs/                   # ドキュメント
```

## 🔧 技術スタック

### バックエンド
- Java 11
- Jakarta EE 8
- Payara Micro
- PostgreSQL
- Maven

### フロントエンド
- Vue 3
- Vue Router
- Axios
- Tailwind CSS
- Vite

### インフラ
- Docker
- Docker Compose
- Dev Containers

## 📚 API エンドポイント

### BFF API (Port 8082)

#### 認証
- `POST /api/login` - ログイン
- `POST /api/logout` - ログアウト
- `GET /api/verify` - トークン検証

#### ユーザー情報
- `GET /api/account` - 自分のアカウント情報取得（認証必要）
- `GET /api/users` - 全ユーザー取得（認証必要）
- `GET /api/users/{id}` - ユーザー情報取得（認証必要）
- `GET /api/users/{id}/points` - ユーザーポイント取得（認証必要）

#### ポイント情報
- `GET /api/points` - 自分のポイント残高取得（認証必要）
- `GET /api/points/history` - 自分のポイント履歴取得（認証必要）

詳細なAPI仕様は各サービスのREADMEを参照してください。

## 🗄️ データベース

### 接続情報

```
Host: localhost
Port: 5432
User: postgres
Password: postgres
```

### データベース一覧

- `user_service_db` - ユーザー情報
- `auth_service_db` - 認証・セッション情報
- `point_service_db` - ポイント情報

## 🛠️ 開発

### 各サービスの再ビルド

```bash
# ユーザーサービス
cd src/user-service && mvn clean package

# 認証サービス
cd src/auth-service && mvn clean package

# ポイントサービス
cd src/point-service && mvn clean package

# BFF
cd src/bff && mvn clean package
```

### フロントエンドの開発

```bash
cd frontend
npm run dev    # 開発サーバー起動
npm run build  # プロダクションビルド
```

## 📖 ドキュメント

- [アーキテクチャ詳細](./docs/architecture.md)
- [User Service README](./src/user-service/README.md)
- [Auth Service README](./src/auth-service/README.md)
- [BFF README](./src/bff/README.md)

## 🤝 貢献

プルリクエストを歓迎します。大きな変更の場合は、まずissueを開いて変更内容を議論してください。

## 📝 ライセンス

MIT
