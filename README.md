...
Java 11 + Payara + PostgreSQL + Vue 3 で構築されたマイクロサービスアーキテクチャの会員管理アプリケーション

## 📋 概要

このアプリケーションは、ユーザー管理と認証機能を持つマイクロサービスで構成されています。ユーザーはログインして自分のアカウント情報とポイント残高を確認できます。
...
```
┌─────────────┐
│  Frontend   │  Vue 3 + Tailwind CSS (Port: 3000)
│  (Vue 3)    │
└──────┬──────┘
       │
       ↓
...
└─────────────┘   └─────────────┘   └─────────────┘   └─────────────┘
       ...
       ↓                 ↓                 ↓
┌──────────────┐   ┌──────────────┐   ┌──────────────┐
│user_service_ │   │auth_service_ │   │point_service_│
│     db       │   │     db       │   │     db       │
└──────────────┘   └──────────────┘   └──────────────┘
```
...
**Maven 3.9.9（Java 11対応）とPayara Microをインストールするため、DevContainerの再ビルドが必要です。**
...
**重要**: まずDevContainerを再ビルドしてMaven 3.9.9（Java 11対応）とPayara Microをインストールしてください。
...
| ユーザーID | 氏名 | ユーザー名 | パスワード | ポイント |
|----------|------|-----------|-----------|---------|
| 1 | 田中太郎 | tanaka_taro | password123 | 1500 |
| 2 | 鈴木花子 | suzuki_hanako | password123 | 2300 |
| 3 | 山田次郎 | yamada_jiro | password123 | 500 |
| 4 | 佐藤優希 | sato_yuki | password123 | 3200 |
| 5 | 高橋舞 | takahashi_mai | password123 | 1000 |
...
#### ユーザー情報
- `GET /api/account` - 自分のアカウント情報取得（認証必要）
- `GET /api/users` - 全ユーザー取得（認証必要）
- `GET /api/users/{id}` - ユーザー情報取得（認証必要）
- `GET /api/users/{id}/points` - ユーザーポイント取得（認証必要）

#### ポイント情報
- `GET /api/points` - 自分のポイント残高取得（認証必要）
- `GET /api/points/history` - 自分のポイント履歴取得（認証必要）

詳細なAPI仕様は各サービスのREADMEを参照してください。
...
- `user_service_db` - ユーザー情報
- `auth_service_db` - 認証・セッション情報
- `point_service_db` - ポイント情報
