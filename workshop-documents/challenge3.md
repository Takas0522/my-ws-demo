...
Custom Agent  `frontend-unit-test-orchestrator` を利用します。

> [!CAUTION]
> 実行時間はかなり長いです。時間がなさそうな場合は別で全体ではなく特定のComponentなどを対象に実行したほうがいいです。

``` markdown
frontendでまだ実装されていない機能のテストを実装してください
<!-- または -->
{特定ファイル}のUnitTestを作成してください。
```
...
> [!CAUTION]
> e2eテストの生成と修正の実行時間はかなり長いです。時間がなさそうな場合は別で全体ではなく特定のComponentなどを対象に実行したほうがいいです。
...
### e2e Test(opt.2:Playwright Test Agents)

- `playwright-test-planner` でプラン作成
  - 時間がない場合は特定のComponentなどを対象にテストのプラン作成と実装を依頼したほうがいいです

> [!CAUTION]
> e2eテストの生成と修正の実行時間はかなり長いです。時間がなさそうな場合は別で全体ではなく特定のComponentなどを対象に実行したほうがいいです。

- `playwright-test-generator` でテスト作成
